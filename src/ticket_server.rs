...
impl TicketServer {
    ...
    pub fn book_tickets(&mut self, user_name: &str, amount: u32) -> Result<(), String> {
        info!(
            "Hi, {}! There are {} seats available for {} and you'd like to buy {}.",
            user_name, self.available_seats, self.moovie_name, amount
        );

        if self.available_seats >= amount {
            self.available_seats -= amount;
            info!(
                "Enjoy the movie, {}! Remaining seats: {}",
                user_name, self.available_seats
            );
            Ok(())
        } else {
            error!(
                "Sorry {}, we have {} seats left.",
                user_name, self.available_seats
            );
            Err(format!(
                "Sorry {}, we have only {} seats left",
                user_name, self.available_seats
            ))
        }
    }
